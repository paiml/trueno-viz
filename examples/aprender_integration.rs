...
fn main() {
    ...
    println!(
        "   Saved: aprender_histogram.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
    ...
    let fb = predictions
        .residual_plot(&actual)
        .expect("Failed to create residual plot");
    PngEncoder::write_to_file(&fb, "aprender_residuals.png").expect("Failed to write PNG");
    println!(
        "   Saved: aprender_residuals.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
    ...
    println!(
        "   Saved: aprender_loss_curve.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
    ...
    let columns = vec![
        ...
        (
            "feature_2".to_string(),
            Vector::from_slice(&[2.1, 3.9, 6.2, 7.8, 10.1, 11.9]),
        ),
        (
            "target".to_string(),
            Vector::from_slice(&[0.5, 1.2, 1.8, 2.5, 3.1, 3.8]),
        ),
    ];
    ...
    println!(
        "   Saved: aprender_df_scatter.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
    ...
    println!(
        "   Saved: aprender_df_histogram.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
    ...
    let fb = df
        .boxplot(&["feature_1", "feature_2"])
        .expect("Failed to create boxplot");
    PngEncoder::write_to_file(&fb, "aprender_df_boxplot.png").expect("Failed to write PNG");
    println!(
        "   Saved: aprender_df_boxplot.png ({}x{})\n",
        fb.width(),
        fb.height()
    );
