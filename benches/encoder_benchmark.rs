...
fn png_encoder_benchmark(c: &mut Criterion) {
    ...
    for (width, height) in [(800, 600), (1920, 1080), (3840, 2160)] {
        ...
        group.bench_with_input(
            ...
            &(width, height),
            |b, _| {
                b.iter(|| PngEncoder::to_bytes(black_box(&fb)).unwrap());
            },
        );
    }
...
fn svg_encoder_benchmark(c: &mut Criterion) {
    ...
    for (width, height) in [(800, 600), (1920, 1080)] {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let encoder = SvgEncoder::from_framebuffer(black_box(&fb)).unwrap();
                    encoder.render()
                });
            },
        );
...
fn png_compression_benchmark(c: &mut Criterion) {
    ...
    let mut fb_solid = Framebuffer::new(width, height).unwrap();
    fb_solid.clear(Rgba::RED);

    // Gradient (moderately compressible)
    let mut fb_gradient = Framebuffer::new(width, height).unwrap();
    for y in 0..height {
        for x in 0..width {
            let r = ((x as f32 / width as f32) * 255.0) as u8;
            let g = ((y as f32 / height as f32) * 255.0) as u8;
    ...
    let mut fb_noise = Framebuffer::new(width, height).unwrap();
    for y in 0..height {
        for x in 0..width {
    ...
    group.bench_function("solid", |b| {
        b.iter(|| PngEncoder::to_bytes(black_box(&fb_solid)).unwrap());
    });

    group.bench_function("gradient", |b| {
        b.iter(|| PngEncoder::to_bytes(black_box(&fb_gradient)).unwrap());
    });

    group.bench_function("noise", |b| {
        b.iter(|| PngEncoder::to_bytes(black_box(&fb_noise)).unwrap());
    });
